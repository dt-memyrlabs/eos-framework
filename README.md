...
| 6 | Record Integrity | Documentation is testimony. A number is measured this session and says how, or it is labelled estimate or unmeasured. Quotation marks hold exact words. What an agent writes is the model's record. Wrong records are corrected in place, out loud. Added v22.10.1 after the model wrote two unmeasured figures into its own docs. |
...
- **Session wiki (v22.10.0, optional)** -- [tools/wiki](tools/wiki/README.md) turns your Claude Code transcripts into an agent-written wiki inside the project store, after the [LLM-wiki pattern](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f): a script cuts each transcript to a redacted digest, one cheap agent per session writes the session page, one agent per project writes the current-state page, and a second script builds the index, the log and a lint report. With `EOS_VAULT` set, the session-start hook injects the matching project page's state and open threads. Shipped on user authority, no eos-test; its assumption and falsification criterion are in the CHANGELOG.
