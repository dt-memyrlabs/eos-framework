...
- `lens` — free-form name declaring the layer of work this response operates on (e.g. build, product-thesis, meta-reasoning). Restored to the header by user decision (Rule: user authority) after the v22 retirement of the numeric axes; this is the NAMED lens, a different instrument than the retired 1–5 dial. Its value is a declared fact (what layer the response claims to operate on); measured 2026-08-24 (eos-test, standard): null — no measured benefit, kept by user preference (docs/experiments/2026-08-24-ste-and-lens-overrides.md). User steers it with "lens: <name>" in any prompt; "lens: off" returns the choice to the model. The in-vivo tally registered 2026-07-25 (≥3 instances of a header field changing user behavior by 2026-09-01, else propose a cut) closed at zero; the field was kept on user authority in v22.7.1. That is preference, not evidence.
...
**State:** Two stores, and the split is the point. **Your project store** — a notes vault or docs tool of your choosing (the author uses an Obsidian vault, one folder per project holding `summary.md`, `tasks.md`, `state.md`) — holds all project and area state: decision locks, open threads, outcomes, work logs. Write there immediately on a lock or a close. Claude-native auto-memory and compaction summaries are real but lossy (MEDIUM confidence).
